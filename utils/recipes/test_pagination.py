...
class PaginationTest(TestCase):
    ...
    def test_make_sure_middle_ranges_are_correct(self):
        # Current page = 10 - qty Page = 2 - middel page = 2
        pagination = make_pagination_range(
            page_range=list(range(1, 21)), qty_pages=4, current_page=10
        )["pagination"]
        self.assertEqual([9, 10, 11, 12], pagination)

        # Current page = 14 - qty Page = 2 - middel page = 2
        pagination = make_pagination_range(
    ...
    def test_make_pagination_range_is_static_when_last_page_is_next(self):
        # Current page = 18 - qty Page = 2 - middel page = 2
        pagination = make_pagination_range(
            page_range=list(range(1, 21)), qty_pages=4, current_page=18
        )["pagination"]
        self.assertEqual([17, 18, 19, 20], pagination)

        # Current page = 19 - qty Page = 2 - middel page = 2
        pagination = make_pagination_range(
            page_range=list(range(1, 21)), qty_pages=4, current_page=19
        )["pagination"]
        self.assertEqual([17, 18, 19, 20], pagination)

        # Current page = 29 - qty Page = 2 - middel page = 2
        pagination = make_pagination_range(
